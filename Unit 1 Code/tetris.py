    ...
    for i in range(len(piece.blocks) - 1, -1, -1):
        for row in rows:
            if piece.blocks[i][row] == 1:
                ...
                if changex + spawnx + i + 1 >= gridwidth or changey + spawny + row < gridheight and \
                        grid[changey + spawny + row][changex + spawnx + i + 1].value:
                    return False
                rows.remove(row)
                continue
...
def checkleft(piece, changex, changey):
    ...
    for i in range(len(piece.blocks)):
        for row in rows:
            if piece.blocks[i][row] == 1:
                if changex + spawnx + i <= 0 or changey + spawny + row < gridheight and grid[changey + spawny + row][
                    changex + spawnx + i - 1].value:
                    return False
                rows.remove(row)
...
def setpiece(piece):
    ...
    for i in range(len(piece.blocks)):
        for j in range(len(piece.blocks[i])):
            if (spawnx + i + changex < gridwidth and spawnx + i + changex >= 0) and (
                    spawny + j + changey < gridheight and spawny + j + changey >= 0) and piece.blocks[i][j]:
...
def deletepiece(piece):
    ...
    for i in range(len(piece.blocks)):
        for j in range(len(piece.blocks[i])):
            if (spawnx + i + changex < gridwidth and spawnx + i >= 0) and (
                    spawny + j + changey < gridheight and spawny + j >= 0) and piece.blocks[i][j]:
    ...
    for j in range(len(currentpiece.blocks) - 1, -1, -1):
        for i in range(len(currentpiece.blocks[j])):
            if currentpiece.blocks[j][i] == 1:
                rect(startx + (j + changex + spawnx) * blocksize, starty - (i + changey + spawny) * blocksize,
                     blocksize, blocksize)
